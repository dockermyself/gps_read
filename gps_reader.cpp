...
bool GPSReader::ParseGGA(const std::string buf, LLA &lla)
{
    ...
    SetLLA(lla, timestamp, latitude, longitude, altitude, quality, "$GGA");

    return true;
}

//$GPRMC,121252.000,A,3958.3032,N,11629.6046,E,15.15,359.95,070306,,,A*54
bool GPSReader::ParseRMC(const std::string buf, LLA &lla)
{
    std::vector<std::string> str_vecs;
    ...
    SetLLA(lla, timestamp, latitude, longitude, 0, status, "$RMC");
    return true;
}
